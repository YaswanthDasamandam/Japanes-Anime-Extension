...
(function () {
  ...
  /**
   * De-inflects an inflected Japanese surface word back to possible dictionary forms (lemmas).
   * @param {string} word - The inflected surface form (e.g. '安くてさ', '食べさせる', '思っている')
   * @returns {Array<{ lemma: string, suffix: string, replace: string, tag: string, particle: string }>}
   */
  function deinflect(word) {
    ...
    // 1. Strip trailing sentence particles (e.g. '安くてさ' -> '安くて', '思うよね' -> '思う')
    ...
    // 2. Apply morphological de-inflection rules to each candidate
