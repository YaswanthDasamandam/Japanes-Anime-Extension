...
/**
 * Built-in local offline dictionary for Japanese learners and anime dialogue.
 * Includes core vocabulary, JLPT levels (N5-N1), particles, pronouns, and anime terminology.
 */
export const LOCAL_DICTIONARY = {
  ...
  // Verbs (Dictionary Root Forms & Conjugations)
  ...
  'どうして': { kana: 'どうして', romaji: 'doushite', meanings: ['why, how come'], pos: 'adverb', jlpt: 'N5' },
...
// Build reverse indexes for romaji and kana search
...
for (const [key, entry] of Object.entries(LOCAL_DICTIONARY)) {
  ...
  if (!romajiIndex.has(normRomaji)) {
    romajiIndex.set(normRomaji, []);
  }
  romajiIndex.get(normRomaji).push({ kanji: key, ...entry });

  const normKana = entry.kana.replace(/[\s\-_()]/g, '');
  if (!kanaIndex.has(normKana)) {
    kanaIndex.set(normKana, []);
  }
  kanaIndex.get(normKana).push({ kanji: key, ...entry });
}

/**
 * Looks up a word definition by Kanji, Kana, or Romaji.
 * @param {string} query - The word to search (e.g. "omae", "お前", "shinde", "死ぬ")
 * @returns {Array<Object>} - List of matching dictionary entries
 */
export function lookupWord(query) {
  if (!query) return [];
  const clean = query.trim();

  // 1. Direct Kanji/Word match
  if (LOCAL_DICTIONARY[clean]) {
    return [{ kanji: clean, ...LOCAL_DICTIONARY[clean] }];
  }

  // 2. Normalized Romaji lookup
  const normRomaji = clean.toLowerCase().replace(/[\s\-_]/g, '');
  if (romajiIndex.has(normRomaji)) {
    return romajiIndex.get(normRomaji);
  }

  // 3. Kana lookup (converting Romaji to Hiragana if needed)
  const hiragana = wanakana.toHiragana(clean);
  if (LOCAL_DICTIONARY[hiragana]) {
    return [{ kanji: hiragana, ...LOCAL_DICTIONARY[hiragana] }];
  }
  if (kanaIndex.has(hiragana)) {
    return kanaIndex.get(hiragana);
  }

  // 4. Verb De-inflection rules (e.g. 'shinde' -> 'shinu', 'kaite' -> 'kaku')
  const deinflected = [];
  if (normRomaji.endsWith('nde')) {
    deinflected.push(normRomaji.slice(0, -3) + 'nu'); // shinde -> shinu
    deinflected.push(normRomaji.slice(0, -3) + 'mu'); // yonde -> yomu
    deinflected.push(normRomaji.slice(0, -3) + 'bu'); // asonde -> asobu
  } else if (normRomaji.endsWith('ite')) {
    deinflected.push(normRomaji.slice(0, -3) + 'ku'); // kaite -> kaku
  } else if (normRomaji.endsWith('tte')) {
    deinflected.push(normRomaji.slice(0, -3) + 'u');  // katte -> kau
    deinflected.push(normRomaji.slice(0, -3) + 'tsu'); // matte -> matsu
    deinflected.push(normRomaji.slice(0, -3) + 'ru'); // totte -> toru
  } else if (normRomaji.endsWith('shite')) {
    deinflected.push(normRomaji.slice(0, -5) + 'suru'); // benkyou shite -> benkyou suru
  } else if (normRomaji.endsWith('masu') || normRomaji.endsWith('mashita')) {
    const stem = normRomaji.replace(/mash?i?ta?|masu/, '');
    deinflected.push(stem + 'ku');
    deinflected.push(stem + 'ru');
    deinflected.push(stem + 'u');
  }

  for (const candidate of deinflected) {
    if (romajiIndex.has(candidate)) {
      return romajiIndex.get(candidate);
    }
  }

  // 5. Prefix / Stem match (e.g. "samuidesu" -> "samui")
  for (const [romKey, entries] of romajiIndex.entries()) {
    if (normRomaji.startsWith(romKey) && romKey.length >= 3) {
      return entries;
    }
  }
